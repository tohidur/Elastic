...
def autocomplete_view(request):
    ...
    options = resp['first_name'][0]['options']
...
def student_detail(request):
    ...
    return render(request, 'student-detais.html', context={'student': student})
...
class HomePageView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        search_result = client.search(index='django', doc_type='student',
                                      body=body)
        context = super(HomePageview, self).gen_context_data(**kwargs)
        context['hits'] = [
            self.convert_hit_to_template(c) for c in search_result['hits']['hits']
        ]
        context['aggregations'] = self.prepare_facet_data(
            search_result['aggregations'], self.request.GET)
        return context
